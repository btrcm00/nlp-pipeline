...
from common.config import PipelineConfig, ONNXOptions
from common.base import BaseModel
from pipeline.dataset_manager.dataset import PipelineDataset
from pipeline.model_manager.model import PipelineModel
...
class Metrics(BaseModel):
    ...
    def generate_metrics(self, predictions, references) -> Dict:
        raise "This func need implementation"


class Processor(BaseModel):
    def __init__(self, config: PipelineConfig = None, model_class: str = None):
        super(Processor, self).__init__(config=config)

        self.converter = ONNXConverter(mode="ONNXRUNTIME", config=self.config)
        self.model = PipelineModel(config=self.config)
        self.dataset = PipelineDataset
        self.metrics = Metrics(task_type=None)
    ...
    def compute_metrics(self, eval_predictions: EvalPrediction):
        ...
        output = self.metrics.generate_metrics(predictions=predictions, references=labels)
        return output
    ...
    def export_model_to_onnx(self, checkpoint_path: str = None, use_gpu: bool = False, adapter_path: str = None):
        def _get_dummy_inputs(example):
            _example: str = self.dataset.get_dummy_example(example)
            _dummy_inputs = self.tokenizer(_example, return_tensors="pt", padding=True)
        ...
        dummy_inputs = _get_dummy_inputs(example=None)
    ...
    def k_fold_training(self, n_folds: int = 5, use_lora: bool = False):
        train_dataset = self.dataset(config=self.config, mode="train", tokenizer=self.tokenizer, num_folds=n_folds)
        for fold_id in range(n_folds):
            ...
            eval_fold: PipelineDataset = train_dataset.apply_fold(fold_id=fold_id)
            self.init_trainer(output_dir=fold_output_dir, train_dataset=train_dataset, eval_dataset=eval_fold)
            self.logger.info(f"TRAINING: {len(self.trainer.train_dataset)}, EVAL: {len(self.trainer.eval_dataset)}")
            self.trainer.train()
    ...
    def train(self, use_lora: bool = False, export_last=False, export_mode: str = "last", n_folds: int = None):
        ...
        if n_folds is not None:
            ...
            train_dataset = self.dataset(config=self.config, mode="train", tokenizer=self.tokenizer,
                                         num_folds=n_folds)
            eval_dataset = self.dataset(config=self.config, mode="eval", tokenizer=self.tokenizer,
                                        num_folds=n_folds)
...
if __name__ == "__main__":
    trainer = Processor(model_class="roberta_linear")
